...
while {alive player && dialog} do {
			...
			if (!_isAll) then {
				...
				
				{
					...
					if (alive (leader _selected) && isPlayer(leader _selected)) then {
						...
						[getPlayerUID(leader _selected), "SetTask", [_taskType,_taskTime,_taskTimeLabel,_position]] Call SendToClients;
					};
				} forEach _teams;
			} else {
				player kbTell [sideHQ, WFBE_V_HQTopicSide, "OrderSentAll",["1","","All",["all"]],["2","","moving to position",["HC_MovingToPosition"]],["3","","over.",["Over1"]],true];
				...
				[sideJoined, "SetTask", [_taskType,_taskTime,_taskTimeLabel,_position]] Call SendToClients;
			};
		};
	};
	...
	if (MenuAction == 303) then {
		...
		["RequestTeamUpdate", [_to,_behavior,_combat,_formation,_speed]] Call SendToServer;
	};	
