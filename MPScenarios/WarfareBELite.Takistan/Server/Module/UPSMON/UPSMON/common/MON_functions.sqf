...
MON_deleteDeadDist = {private["_u","_s","_dist","_OCercanos","_cicle","_deleted","_isplayer"];
	...
	while {!_deleted} do {
		...
		_OCercanos = nearestObjects [getPos _u, ["Man"] , _dist];
...
MON_NearestLandTransport = {
	...
	//Buscamos objetos cercanos
	_OCercanos = nearestObjects [getPos _npc, ["Car","TANK","Truck","Motorcycle"] , 150];
...
MON_NearestsLandTransports = {
	...
	//Buscamos objetos cercanos
	_OCercanos = nearestObjects [getPos _npc, ["Car","TANK","Truck","Motorcycle"] , _distance];
...
MON_NearestsAirTransports = {
	...
	//Buscamos objetos cercanos
	_OCercanos = nearestObjects [getPos _npc, ["Helicopter"] , _distance];
...
MON_NearestsStaticWeapons = {
	...
	_distance = _this select 1;
		...
		//Buscamos objetos cercanos
		_OCercanos = nearestObjects [getpos _npc, ["StaticWeapon"] , _distance];
...
MON_Nearestsboats = {
	...
	_distance = _this select 1;
		...
		//Buscamos objetos cercanos
		_OCercanos = nearestObjects [getPos _npc, ["Ship"] , _distance];
...
MON_assignasgunner = {
	...
	waituntil {sleep 0.05;_gunner != vehicle _gunner || !alive _gunner || !canmove _gunner ||!alive _vehicle || !canfire _vehicle};
...
MON_doGetOut = {
	...
	waituntil {sleep 0.05;!alive _npc || !canmove _npc || !alive _vehicle || ( (abs(velocity _vehicle select 0)) <= 0.1 && (abs(velocity _vehicle select 1)) <= 0.1 )
	...
	waituntil {sleep 0.05;!alive _npc || !canmove _npc || vehicle _npc == _npc};
	if (!alive _npc || !canmove _npc) exitwith{};
...
MON_doParadrop = {
	...
	waituntil {sleep 0.05;count crew _heli > 0 || !alive _heli || !canmove _heli};
	...
	if (((position _heli) select 2) >= 90 && !surfaceIsWater position _heli && (!(toupper (behaviour _npc) IN _landonBeh) || (random 100) < 20)) then {
		...
		_timeout = time + 60;
		{
			waituntil {sleep 0.05;(_x == vehicle _x ) || !alive _x || !canmove _x || isnull _x || time > _timeout};
...
MON_landHely = {
	...
	waituntil {sleep 0.05;!alive _heli || toUpper(landResult _heli) != "NOTREADY" };
	...
	_timeout = 60 + time;
	waituntil {sleep 0.05;!alive _heli || time > _timeout || ((abs(velocity _heli select 2)) <= 1 && ((position _heli) select 2) <= 0.7)};
	...
	if (((position _heli) select 2) > 2 && ((position _heli) select 2) < 30 && !surfaceiswater position _heli) then {
		_heli land "LAND";
		_timeout = 30 + time;
		waituntil {sleep 0.05;!alive _heli || time > _timeout || ((position _heli) select 2) > 30 || ( (abs(velocity _heli select 2)) <= 1 && ((position _heli) select 2) <= 0.7)};
	...
	{
		waituntil {sleep 0.05;vehicle _x == _x || !canmove _x || !alive _x || movetofailed _x  || time > _timeout };
	} forEach _jumpers;
...
MON_movetoDriver = {
	...
	if (isnull(driver _vehicle) || !alive(driver _vehicle) || !canmove(driver _vehicle)) then {
		 _npc action ["getOut", _vehicle];
		 doGetOut _npc;
		WaitUntil {sleep 0.05;vehicle _npc==_npc || !alive _npc || !canmove _npc};
		//Si está muerto
		if (!alive _npc || !canmove _npc) exitwith{};
		unassignVehicle _npc;
...
MON_GetNearestBuildings = {
	...
	_OCercanos = nearestObjects [getPos _object, ["house","building"] , _distance];
...
MON_movetoBuilding = {
	...
	waitUntil {sleep 0.05;moveToCompleted _npc || moveToFailed _npc || !alive _npc || !canmove _npc || _timeout < time};
...
MON_patrolBuilding = {
	...
	if (!_inbuilding) then {
		...
		while { _i <= _patrolto && alive _npc && canmove _npc} do{
			...
			_time = time + 30;
			waitUntil {sleep 0.05;moveToCompleted _npc or moveToFailed _npc or !alive _npc or _time < time};
...
MON_doCreateMine = {
	...
	if ((count _this) > 1) then {
		...
		waituntil {sleep 0.05;unitReady _soldier || moveToCompleted _soldier || moveToFailed _soldier || !alive _soldier || !canmove _soldier};
	};
...
MON_surrender = {
	...
	if ( _npc != _vehicle || !(_npc iskindof "Man" )) then {
		...
		waituntil {sleep 0.05;_npc == vehicle _npc || !alive _npc};
	};
...
MON_ACE_Watersurvival = {
	...
	waitUntil {sleep 0.05;!canmove _ejector || !alive _ejector || isnull (_ejector) || ((getPos vehicle _ejector) select 2 < 1) };
	...
	waitUntil {sleep 0.05;!canmove _ejector || !alive _ejector || isnull (_ejector) || !surfaceIsWater (position _ejector) };
...
MON_deadbodies = {
	...
	//_side = _this select 2;
		...
		//Buscamos objetos cercanos
		_OCercanos = nearestObjects [getPos _npc, ["Man"] , _distance];
