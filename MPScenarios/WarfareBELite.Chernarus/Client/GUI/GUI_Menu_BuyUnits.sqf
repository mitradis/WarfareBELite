...
while {alive player && dialog} do {
	...
	//--- Lock icon.
	if (MenuAction == 401) then {if (paramVehLock) then {MenuAction = -1;_isLocked = if (_isLocked) then {false} else {true};_updateDetails = true}};
	...
	if (_updateList) then {
		switch (_type) do {
			//--- Specials.
			case 'Depot': {
				_sorted = [[vehicle player, ('WFBE_TOWNPURCHASERANGE' Call GetNamespace)] Call GetClosestDepot];
			};
			case 'Airport': {
				_sorted = [[vehicle player, ('WFBE_PURCHASEUNITSRANGEAIR' Call GetNamespace)] Call GetClosestAirport];
			};
			//--- Factories
			default {
		...
		lbClear 12018;
		{
			_nearTown = ([_x, towns] Call GetClosestEntity) getVariable 'name';
