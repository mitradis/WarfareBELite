...
_backpack = (_target) Call GetUnitBackpack;
...
_targets = (_target) Call UI_Gear_UpdateTarget;
...
while {true} do {
	...
	if (_funds_cli != _funds) then {
		_funds_cli = _funds;
		((uiNamespace getVariable "wfbe_display_buygear") displayCtrl 503011) ctrlSetStructuredText (parseText Format [localize "STR_WF_INFO_Funds", _funds]);
	};
	...
	if (_click_pool_main != -1) then {uiNamespace setVariable ['wfbe_display_buygear_pool_main', -1]; _size = (_gear_mag_main) Call GetMagazinesSize; if (_click_pool_main <= _size && _size > 0) then {_gear_mag_main = [_gear_mag_main, _click_pool_main] Call RemoveMagazineGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_main"];_update_inventory = true;_has_inv_changed = true;}};
	if (_click_pool_gun != -1) then {uiNamespace setVariable ['wfbe_display_buygear_pool_gun', -1]; _size = (_gear_mag_pool) Call GetMagazinesSize; if (_click_pool_gun <= _size && _size > 0) then {_gear_mag_pool = [_gear_mag_pool, _click_pool_gun] Call RemoveMagazineGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_hand"];_update_inventory = true;_has_inv_changed = true;}};
	...
	if (WFBE_MenuAction == 901) then {WFBE_MenuAction = -1;if (_gear_primary != "") then {_gear_mag_main = [_gear_primary, "", _gear_mag_main] Call ReplaceMagazinesGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_sel_weapons = _gear_sel_weapons - [_gear_primary];_gear_primary = ""; _gear_refresh = ["weapons","magazines_main"]; _update_inventory = true;_has_inv_changed = true;}};
	if (WFBE_MenuAction == 902) then {WFBE_MenuAction = -1;if (_gear_backpack != "") then {_gear_sel_weapons = _gear_sel_weapons - [_gear_backpack];_gear_backpack = "";_gear_backpack_content=[[[],[]],[[],[]]];_gear_refresh = ["weapons"];[_gear_backpack,_target] Call UI_Gear_UpdateView;_update_inventory = true;};if (_gear_secondary != "") then {_gear_mag_main = [_gear_secondary, "", _gear_mag_main] Call ReplaceMagazinesGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool;_gear_sel_weapons = _gear_sel_weapons - [_gear_secondary];_gear_secondary = ""; _gear_refresh = ["weapons","magazines_main"]; _update_inventory = true;_has_inv_changed = true;}};
	if (WFBE_MenuAction == 903) then {WFBE_MenuAction = -1;if (_gear_pistol != "") then {_gear_mag_pool = [_gear_pistol, "", _gear_mag_pool] Call ReplaceMagazinesGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool;_gear_sel_weapons = _gear_sel_weapons - [_gear_pistol];_gear_pistol = ""; _gear_refresh = ["weapons","magazines_hand"]; _update_inventory = true;_has_inv_changed = true;}};
	...
	if (WFBE_MenuAction == 505) exitWith {WFBE_MenuAction = -1;closeDialog 0;createDialog "WF_Menu";
	};
	...
	if (_update_target) then {
		...
		if !(alive _target) then {
			hint localize "STR_WF_Target_Dead";
			_update_target = true;
			_target = player;
			_targets = (_target) Call UI_Gear_UpdateTarget;
		};
		...
		if (_target isKindOf "Man") then {
			...
			_target_weapons = [weapons _target, "weapons"] Call UI_Gear_Sanitize;
			_target_magazines = [magazines _target, "magazines"] Call UI_Gear_Sanitize;
			_target_cancarrybp = if (getNumber(configFile >> 'CfgVehicles' >> typeOf _target >> 'canCarryBackPack') == 1) then {true} else {false};
			_backpack = (_target) Call GetUnitBackpack;
			if (_backpack != "") then {_target_weapons = _target_weapons + [_backpack]};
			_returned = [_target_weapons, _target_magazines, _target] Call GetParsedGear;
			...
			_gear_sel_vehicle = if (vehicle _target != _target) then {(vehicle _target) Call GetVehicleContent} else {[[[],[]],[[],[]],[[],[]]]};
			_gear_refresh = ["all"];
		} else {
			_target_weapons = [];
			_target_magazines = [];
			...
			_gear_sel_vehicle = (_target) Call GetVehicleContent;
		};
		[_gear_backpack,_target] Call UI_Gear_UpdateView;
	...
	if (_update_tab) then {
		...
		switch (_tab_current) do {
			case 0: {[_lb_main, missionNamespace getVariable Format ["WFBE_%1_Template", WFBE_Client_SideJoinedText]] Call UI_Gear_FillTemplates};
			case 1: {[_lb_main, [[(missionNamespace getVariable Format ["WFBE_%1_Primary", WFBE_Client_SideJoinedText]) + (missionNamespace getVariable Format ["WFBE_%1_Secondary", WFBE_Client_SideJoinedText]) + (missionNamespace getVariable Format ["WFBE_%1_Pistols", WFBE_Client_SideJoinedText]) + (missionNamespace getVariable Format ["WFBE_%1_Equipment", WFBE_Client_SideJoinedText])],[missionNamespace getVariable Format ["WFBE_%1_Magazines", WFBE_Client_SideJoinedText], "Mag_"]]] Call UI_Gear_FillList};
			case 2: {[_lb_main, [[missionNamespace getVariable Format ["WFBE_%1_Primary", WFBE_Client_SideJoinedText]]]] Call UI_Gear_FillList};
			case 3: {[_lb_main, [[missionNamespace getVariable Format ["WFBE_%1_Secondary", WFBE_Client_SideJoinedText]]]] Call UI_Gear_FillList};
			case 4: {[_lb_main, [[missionNamespace getVariable Format ["WFBE_%1_Pistols", WFBE_Client_SideJoinedText]]]] Call UI_Gear_FillList};
			case 5: {[_lb_main, [[(missionNamespace getVariable Format ["WFBE_%1_Equipment", WFBE_Client_SideJoinedText])],[missionNamespace getVariable Format ["WFBE_%1_Magazines", WFBE_Client_SideJoinedText], "Mag_"]]] Call UI_Gear_FillList};
		};
	...
	if (_update_view) then {
		...
		switch (_view) do {
			case "backpack": {{ctrlShow[_x, false]} forEach [503401, 503402, 503403]; for '_i' from 503501 to 503534 do {ctrlShow [_i, false]};{ctrlShow[_x, true]} forEach[_lb_cargo,503006,503007];[_lb_cargo, _gear_backpack_content, true] Call UI_Gear_FillCargoList;_update_backpack = true};
			...
			case "vehicle": {{ctrlShow[_x, false]} forEach [503401, 503402, 503403]; for '_i' from 503501 to 503534 do {ctrlShow [_i, false]};{ctrlShow[_x, true]} forEach[_lb_cargo,503006,503007]; [_lb_cargo, _gear_vehicle_content, true] Call UI_Gear_FillCargoList; _update_vehicle = true;/*todo update target content*/};
		};
	};
			...
			case "gear": {
				...
				if (isNil '_get') then {
					...
					if (_target != player) then {
						_gear_config = (typeOf _target) Call GetUnitConfigGear;
						...
						if (_gear_backpack != "") then {_gear_sel_weapons = _gear_sel_weapons + [_gear_backpack]; [_gear_backpack,_target] Call UI_Gear_UpdateView};
					} else {
	...
	if (_quick_clear) then {
		_quick_clear = false;
		switch (_view) do {
			case "backpack": {_gear_refresh = ["all"];_gear_backpack_content = [[[],[]],[[],[]]]; _update_backpack = true; _has_inv_changed = true; [_lb_cargo, _gear_backpack_content, true] Call UI_Gear_FillCargoList;};
			...
			case "vehicle": {_gear_vehicle_content = [[[],[]],[[],[]],[[],[]]]; _update_vehicle = true; _has_veh_changed = true;[_lb_cargo, _gear_vehicle_content, true] Call UI_Gear_FillCargoList;};
		};
		_update_inventory = true;
	};
		...
		if (_ui_lnb_currow > -1) then {
			...
			switch (_view) do {
				case "backpack": {
					...
					_gear_backpack_content set [_kind, [_gear_backpack_content select _kind, "substract", if (_kind == 0) then {_item_selected} else {_get select 6}] Call OperateCargoGear];
					[_lb_cargo, _gear_backpack_content, true] Call UI_Gear_FillCargoList;
					_has_inv_changed = true;
				...
				case "vehicle": {
					...
					_gear_vehicle_content set [_kind, [_gear_vehicle_content select _kind, "substract", if (_kind == 1) then {_get select 6} else {_item_selected}] Call OperateCargoGear];
					[_lb_cargo, _gear_vehicle_content, true] Call UI_Gear_FillCargoList;
			...
			if (_tab_current != 0) then {
				...
				if !(isNil '_get') then {
					switch (_view) do {
						case "gear": {
							switch (_belong) do {
								case 0: {_gear_refresh = [];if (_gear_primary != "") then {_gear_mag_main = [_gear_primary, _item_selected, _gear_mag_main] Call ReplaceMagazinesGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_main"]}; _gear_sel_weapons = _gear_sel_weapons - [_gear_primary];_gear_primary = _item_selected; _gear_sel_weapons = _gear_sel_weapons + [_gear_primary]; _gear_refresh = _gear_refresh + ["weapons"];_update_inventory = true;_has_inv_changed = true;};// todo update mags
								case 1: {_gear_refresh = [];if (_gear_pistol != "") then {_gear_mag_pool = [_gear_pistol, _item_selected, _gear_mag_pool] Call ReplaceMagazinesGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_hand"]};_gear_sel_weapons = _gear_sel_weapons - [_gear_pistol];_gear_pistol = _item_selected; _gear_sel_weapons = _gear_sel_weapons + [_gear_pistol]; _gear_refresh = _gear_refresh + ["weapons"];_update_inventory = true;_has_inv_changed = true;};
								case 2: {_add = true;if (_gear_primary != "") then {if ((missionNamespace getVariable _gear_primary) select 4 == 3) then {_add = false}};if (_add) then {_gear_refresh = [];if (_gear_backpack != "") then {_gear_sel_weapons = _gear_sel_weapons - [_gear_backpack];_gear_backpack_content = [[[],[]],[[],[]]];_gear_backpack = "";[_gear_backpack,_target] Call UI_Gear_UpdateView;};if (_gear_secondary != "") then {_gear_mag_main = [_gear_secondary, _item_selected, _gear_mag_main] Call ReplaceMagazinesGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_main"]};_gear_sel_weapons = _gear_sel_weapons - [_gear_secondary];_gear_secondary = _item_selected; _gear_sel_weapons = _gear_sel_weapons + [_gear_secondary]; _gear_refresh = _gear_refresh + ["weapons"];_update_inventory = true;_has_inv_changed = true;}};
								case 3: {_gear_refresh = [];if (_gear_backpack != "") then {_gear_sel_weapons = _gear_sel_weapons - [_gear_backpack];_gear_backpack_content = [[[],[]],[[],[]]];_gear_backpack = "";[_gear_backpack,_target] Call UI_Gear_UpdateView;};if (_gear_primary != "") then {if (_gear_secondary != "") then {_gear_mag_main = [_gear_secondary, "", _gear_mag_main] Call ReplaceMagazinesGear};_gear_mag_main = [_gear_primary, _item_selected, _gear_mag_main] Call ReplaceMagazinesGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_main"]};_gear_sel_weapons = _gear_sel_weapons - [_gear_primary, _gear_secondary];_gear_primary = _item_selected; _gear_sel_weapons = _gear_sel_weapons + [_gear_primary]; _gear_secondary = ""; _gear_refresh = _gear_refresh + ["weapons"];_update_inventory = true;_has_inv_changed = true;};
								...
								case 100: {_mag_size = _get select 5; _size = (_gear_mag_pool) Call GetMagazinesSize; if (_mag_size + _size <= 8) then {_gear_mag_pool = _gear_mag_pool + [_get select 6];_gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_hand"]; _has_inv_changed = true; _update_inventory = true;}};
								case 101: {_mag_size = _get select 5; _size = (_gear_mag_main) Call GetMagazinesSize; if (_mag_size + _size <= 12) then {_gear_mag_main = _gear_mag_main + [_get select 6];_gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_main"]; _has_inv_changed = true; _update_inventory = true;}};							
								case 200: {_add = true;if !(_target_cancarrybp) then {_add = false; hint localize "STR_WF_Cannot_Carry_Backpack"};if (_gear_primary != "") then {if ((missionNamespace getVariable _gear_primary) select 4 == 3) then {_add = false}};if (_add) then {_gear_refresh = [];_gear_backpack_content = _get select 5;if (_gear_secondary != "") then {_gear_mag_main = [_gear_secondary, "", _gear_mag_main] Call ReplaceMagazinesGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_main"]; _gear_sel_weapons = _gear_sel_weapons - [_gear_secondary]};_gear_sel_weapons = _gear_sel_weapons - [_gear_backpack];_gear_backpack = _item_selected; _gear_sel_weapons = _gear_sel_weapons + [_gear_backpack]; _gear_refresh = _gear_refresh + ["weapons"];[_gear_backpack,_target] Call UI_Gear_UpdateView;_update_inventory = true;_has_inv_changed = true;}};
								case 201: {_add = true;if !(_target_cancarrybp) then {_add = false; hint localize "STR_WF_Cannot_Carry_Backpack"};if (_gear_primary != "") then {if ((missionNamespace getVariable _gear_primary) select 4 == 3) then {_add = false}};if (_add) then {_gear_refresh = [];_gear_backpack_content = [[[],[]],[[],[]]];if (_gear_secondary != "") then {_gear_mag_main = [_gear_secondary, "", _gear_mag_main] Call ReplaceMagazinesGear; _gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_main"]; _gear_sel_weapons = _gear_sel_weapons - [_gear_secondary]};_gear_sel_weapons = _gear_sel_weapons - [_gear_backpack];_gear_backpack = _item_selected; _gear_sel_weapons = _gear_sel_weapons + [_gear_backpack]; _gear_refresh = _gear_refresh + ["weapons"];[_gear_backpack,_target] Call UI_Gear_UpdateView;_update_inventory = true;_has_inv_changed = true;}};
							};
						};
						case "backpack": {
							_kind = switch (true) do { case (_belong < 6): {0}; case (_belong in [100,101]): {1}; default {-1}};
							if (_kind == -1) exitWith {};
							_returned = [_gear_backpack, _gear_backpack_content] Call GetGearCargoSize;
							_add = false;
							...
							if (_add) then {
								_gear_backpack_content set [_kind, [_gear_backpack_content select _kind, "add", if (_kind == 0) then {_item_selected} else {_get select 6}] Call OperateCargoGear];
								[_lb_cargo, _gear_backpack_content, true] Call UI_Gear_FillCargoList;
								_has_inv_changed = true;
								_update_inventory = true;
								_update_backpack = true;
						...
						case "vehicle": {
							...
							if (_kind == -1) exitWith {};
							_returned = [typeOf (vehicle _target), _gear_vehicle_content] Call GetVehicleCargoSize;
							_add = false;
							switch (true) do {
							...
							if (_add) then {
								_gear_vehicle_content set [_kind, [_gear_vehicle_content select _kind, "add", if (_kind == 1) then {_get select 6} else {_item_selected}] Call OperateCargoGear];
								[_lb_cargo, _gear_vehicle_content, true] Call UI_Gear_FillCargoList;
								_has_veh_changed = true;
								_update_inventory = true;
								_update_vehicle = true;
				...
				if (_view == "gear") then {
					...
					_magazines = (_magazines) Call UI_Gear_ParseTemplateContent;
					
					_returned = [_weapons, _magazines] Call GetParsedGear;
					_gear_primary = _returned select 0;
					_gear_secondary = _returned select 1;
					_gear_pistol = _returned select 2;
					_gear_backpack = _returned select 3;
				...
				switch (_view) do {
					case "gear": {
						switch (_get select 4) do {
							case 100: {_mag_size = _get select 5; _size = (_gear_mag_pool) Call GetMagazinesSize; if (_mag_size + _size <= 8) then {_gear_mag_pool = _gear_mag_pool + [_get select 6];_gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_hand"]; _update_inventory = true;_has_inv_changed = true;}};
							case 101: {_mag_size = _get select 5; _size = (_gear_mag_main) Call GetMagazinesSize; if (_mag_size + _size <= 12) then {_gear_mag_main = _gear_mag_main + [_get select 6];_gear_sel_magazines = _gear_mag_main + _gear_mag_pool; _gear_refresh = ["magazines_main"]; _update_inventory = true;_has_inv_changed = true;}};
						};
					};
					case "backpack": {
						_returned = [_gear_backpack, _gear_backpack_content] Call GetGearCargoSize;
						if (_returned select 1 && ((_returned select 0) >= (_get select 5))) then {
							_gear_backpack_content set [1, [_gear_backpack_content select 1, "add", _get select 6] Call OperateCargoGear];
							[_lb_cargo, _gear_backpack_content, true] Call UI_Gear_FillCargoList;
							_has_inv_changed = true;
							_update_inventory = true;
							_update_backpack = true;
					...
					case "vehicle": {
						...
						if ((_returned select 1) > 0) then {
							_gear_vehicle_content set [1, [_gear_vehicle_content select 1, "add", _get select 6] Call OperateCargoGear];
							[_lb_cargo, _gear_vehicle_content, true] Call UI_Gear_FillCargoList;
							_has_veh_changed = true;
							_update_inventory = true;
							_update_vehicle = true;
	...
	if (_update_backpack) then {
		...
		_returned = [_gear_backpack, _gear_backpack_content] Call GetGearCargoSize;
		_html = Format[localize "STR_WF_GEAR_Backpack",_returned select 5, getNumber(configFile >> 'CfgVehicles' >> _gear_backpack >> 'transportMaxWeapons'),_returned select 3,_returned select 4,getNumber(configFile >> 'CfgVehicles' >> _gear_backpack >> 'transportMaxMagazines'),(_returned select 3)+(_returned select 4),getNumber(configFile >> 'CfgVehicles' >> _gear_backpack >> 'transportMaxMagazines')];
		...
		if (_ui_lnb_currow > -1) then {
			...
			if !(isNil '_get') then {
				if ((_get select 4) in [0,1,2,3,4]) then {
					lnbClear _lb_secondary;
					[_lb_secondary, [[_get select 6, "Mag_"]]] Call UI_Gear_FillList;
				};
			};
		};
	};
	...
	if (_purchase) then {
		_purchase = false;
		//_prices = [[_target_weapons,_gear_sel_weapons],[_target_magazines,_gear_sel_magazines],[_gear_backpack_content,_gear_sel_backpack],[_gear_vehicle_content,_gear_sel_vehicle]] Call UI_Gear_UpdatePrice;
		...
		if (_target isKindOf "Man" && _has_inv_changed) then {
			_msg = _msg + Format["<t color='#B6F563'>%1</t>",[configFile >> 'CfgVehicles' >> typeOf _target, "displayName"] Call GetConfigEntry];
			[_target, _target_weapons - [_gear_backpack], _target_magazines, [_gear_primary,_gear_pistol,_gear_secondary], _gear_backpack, _gear_sel_backpack] Call EquipUnit;
			...
			_gear_cost = [[[],_target_weapons],[[],_target_magazines],[[],_gear_backpack_content],[[],[]]] Call UI_Gear_UpdatePrice;
			_target setVariable ["wfbe_custom_gear_cost", _gear_cost select 0];
		};
		if (_has_veh_changed) then {
			if (_msg != "") then {_msg = _msg + " ,"};
			_msg = _msg + Format["<t color='#B6F563'>%1</t>", [configFile >> 'CfgVehicles' >> typeOf _target, "displayName"] Call GetConfigEntry];
			[vehicle _target, _gear_sel_vehicle] Call EquipVehicle;
		};
		-(_price) Call ChangePlayerFunds;
		if (_has_inv_changed || _has_veh_changed) then {hint parseText Format[localize "STR_WF_INFO_Purchased_Gear",_msg,_price];_price = 0;} else {hint parseText(localize "STR_WF_INFO_NotPurchased_Gear");};
		_has_inv_changed = false;
		_has_veh_changed = false;
		_update_inventory = true;
	};
	...
	if (_template_create) then {
		...
		[_gear_sel_weapons, _gear_sel_magazines, _gear_backpack_content] Call UI_Gear_AddTemplate;
	...
	if (_template_delete) then {
		...
		if (_ui_lnb_currow > -1 && _tab_current == 0 && _view == "gear") then {
			_item_selected = lnbValue[_lb_main,[_ui_lnb_currow, 0]];
			(_item_selected) Call UI_Gear_DeleteTemplate;
			if (_tab_current == 0) then {_update_tab = true};
		} else {
			hint localize "STR_WF_Only_Gear_Tab";
		};
	};
	...
	if (_update_inventory) then {
		...
		_prices = [[_target_weapons,_gear_sel_weapons],[_target_magazines,_gear_sel_magazines],[_gear_sel_backpack, _gear_backpack_content],[_gear_sel_vehicle, _gear_vehicle_content]] Call UI_Gear_UpdatePrice;
		_price = _prices select 0;
		((uiNamespace getVariable "wfbe_display_buygear") displayCtrl 503008) ctrlSetStructuredText (parseText Format[localize "STR_WF_GEAR_MenuCost",if (_price > 0) then {'#F56363'} else {'#76F563'},_price]);
	};
	
	//--- Update the inventory
	if (_update_display) then {
		_update_display = false;
		if (_view == "gear") then {[_gear_sel_weapons, _gear_sel_magazines, _gear_refresh] Call UI_Gear_DisplayInventory};
	};
};
