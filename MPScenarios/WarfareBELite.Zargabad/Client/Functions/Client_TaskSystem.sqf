...
switch (_type) do {
	...
	case "TownAddComplete": {
		...
		for '_i' from 0 to count(_towns)-1 do {
			...
			if (_sideID != sideID) then {
				_task setSimpleTaskDescription [Format[localize "STR_WF_TaskTown",_town getVariable "name"], Format [localize "STR_WF_CHAT_TaskTown_Capture",_town getVariable "name"], Format [localize "STR_WF_CHAT_TaskTown_Capture",_town getVariable "name"]];
			} else {
				_task setSimpleTaskDescription [Format[localize "STR_WF_TaskTown_Complete",_town getVariable "name"], Format [localize "STR_WF_CHAT_TaskTown_Protect",_town getVariable "name"], Format [localize "STR_WF_CHAT_TaskTown_Protect",_town getVariable "name"]];
	...
	case "TownUpdate": {
		...
		if !(isNil '_task') then {
			if (_sideID == sideID) then {
				_task setTaskState "Succeeded";
				_task setSimpleTaskDescription [Format[localize "STR_WF_TaskTown_Complete",_location getVariable "name"], Format [localize "STR_WF_CHAT_TaskTown_Protect",_location getVariable "name"], Format [localize "STR_WF_CHAT_TaskTown_Protect",_location getVariable "name"]];
			} else {
				_task setTaskState "Created";
				_task setSimpleTaskDescription [Format[localize "STR_WF_TaskTown",_location getVariable "name"], Format [localize "STR_WF_CHAT_TaskTown_Capture",_location getVariable "name"], Format [localize "STR_WF_CHAT_TaskTown_Capture",_location getVariable "name"]];
			};
		}
	};

	//--- Hint for a new town task.
	case "TownHintNew": {taskHint [format [localize "str_taskNew" + "\n%1",Format [localize "STR_WF_CHAT_TaskTown_Capture",_location getVariable "name"]], [1,1,1,1], "taskNew"]};
	
	//--- Hint for a completed town task.
	case "TownHintDone": {taskHint [format [localize "str_taskAccomplished" + "\n%1",Format [localize "STR_WF_CHAT_TaskTown_Protect",_location getVariable "name"]], [1,1,1,1], "taskDone"]};
};