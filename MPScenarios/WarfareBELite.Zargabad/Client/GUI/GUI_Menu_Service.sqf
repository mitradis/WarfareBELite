...
if (count _sp > 0) then {
	_csp = [vehicle player,_sp] Call GetClosestEntity;
};
...
_i = 0;
{
	...
	//--- Service Point.
	if (count _sp > 0) then {
		_closestSP = [_x,_sp] Call GetClosestEntity;
	...
	//--- Depots.
	_nObject = [_x, ('WFBE_SUPPORTRANGE' Call GetNamespace)] Call GetClosestDepot;
