...
if (paramBaseArea) then {
	if (_source == _hq) then {
		if (count(WF_Logic getVariable Format ['%1Area',SideJoinedText]) >= ('WFBE_BASEAREAMAX' Call GetNamespace)) then {
			_closest = [_startPos,WF_Logic getVariable Format ['%1Area',SideJoinedText]] Call GetClosestEntity;
			if (_source distance _closest > ('WFBE_MHQBUILDINGRANGE' Call GetNamespace)) exitWith {_tooFar = true};
		};
	};
};
...
_logic spawn {
	_logic = _this;
	waitUntil {sleep 0.05;!isNil {_this getVariable "BIS_COIN_fundsOld"}};
...
waitUntil {sleep 0.05;scriptDone _createBorderScope};
endLoadingScreen;
...
while {!isNil "BIS_CONTROL_CAM"} do {
	...
	if (isnull (uiNamespace getVariable 'wfbe_title_coin') && !_loaded) then { //---TEST
		cameraEffectEnableHUD true;
		1122 cutrsc ["constructioninterface","plain"];
		_loaded = true;
		_logic setVariable ["BIS_COIN_restart",true];
	...
	if (_mode in ["mousemoving","mouseholding"]) then {
		...
		if (count _params > 0) then {
			...
			if (typeof _preview != _itemclass_preview) then {
				...
				if (_index == 0 && _hqDeployed) exitWith {
					...
					["RequestStructure", [sideJoined,_itemclass,[0,0,0],0]] Call SendToServer;
					...
					[] Spawn {
						_start = time;
						waitUntil {sleep 0.05;!((sideJoinedText) Call GetSideHQDeployed) || time - _start > 15};
				...
				[_logic,_itemclass,_pos,_dir,_params] spawn {
					...
					[_logic,_itemclass] call {
						...
						if (_index != -1) then {
							...
							if (_index == 0) then {
								...
								["RequestChangeScore", [player,score player + ('WFBE_COMMANDERBUILDSCORE' Call GetNamespace)]] Call SendToServer;
							};
						};
					...
					[_logic,_itemclass,_pos,_dir,_par] call {    
						...
						if (_find != -1) then {
							...
							["RequestStructure", [sideJoined,_class,_pos,_dir]] Call SendToServer;
						};
						
						if (_class in _defenses) then {
							["RequestDefense", [sideJoined,_class,_pos,_dir,manningDefense]] Call SendToServer;
							lastBuilt = _par;
						};
					};
				};
		...
		if (!([_cashValues,_cashValuesOld] call bis_fnc_arraycompare) || _restart) then {
			...
			if (_upgrades select 13 < 1) then {_categories = _categories - [localize 'STR_WF_Ammo']}; 
