...
for '_i' from 1 to _vehicle_count do {
	...
	_vehicle addEventHandler ['killed', Format["[_this select 0, _this select 1, %1] Spawn UnitKilled",_sideID]];
	_vehicle setVehicleInit Format["[this, %1] ExecVM 'Common\Init\Init_Unit.sqf';", _sideID];
	[_vehicles, _vehicle] Call ArrayPush;
	
	//--- Spawn the pilot.
	_pilot = [_vehicle_pilot, _grp, [100,12000,0], _side] Call CreateMan;
...
_paratroopers = [];
{
	...
	[_paratroopers, _unit] Call ArrayPush;
