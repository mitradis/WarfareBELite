...
_addToListFee = [0,150000,9500,3500,8500,0,if (side player == west) then {if (('WFBE_RESTRICTIONADVAIR' Call GetNamespace) != 2) then {10500} else {7500}} else {5500},0,0];
...
while {alive player && dialog} do {
	...
	if (_ft > 0) then {
		...
		if (time - _lastUpdate > 15 && _currentLevel > 0) then {
			...
			if (player distance _base < _ftr && alive _base && vehicle player != _base && _isDeployed) then {
				...
				_closest = [player,towns] Call GetClosestEntity;
		...
		switch (_currentSpecial) do {
			...
			case "ICBM": {
				if (paramICBM) then {
					...
					_currentLevel = _currentUpgrades select 10;
			...
			case "Paradrop_Ammo": {
				_currentLevel = _currentUpgrades select 14;
	...
	if (mouseButtonUp == 0) then {
		...
		if (MenuAction == 3) then {
			...
			if (!surfaceIsWater _callPos) then {
				...
				["RequestSpecial", ["Paratroops",sideJoined,_callPos,clientTeam]] Call SendToServer;
		...
		if (MenuAction == 7) then {
			...
			[17022] Call SetControlFadeAnimStop;
			_callPos = _map PosScreenToWorld[mouseX,mouseY];
			_destination = [_callPos,_FTLocations] Call GetClosestEntity;
			if (_callPos distance _destination < 500) then {
				...
				waitUntil {sleep 0.05;mapAnimDone || !alive player};
				_skip = false;
		...
		if (MenuAction == 9) then {
			...
			-_currentFee Call ChangePlayerFunds;
			_callPos = _map PosScreenToWorld[mouseX,mouseY];
			["RequestSpecial", ["ParaVehi",sideJoined,_callPos,clientTeam]] Call SendToServer;
		};
		//--- Ammo Paradrop.
		if (MenuAction == 10) then {
			...
			-_currentFee Call ChangePlayerFunds;
			_callPos = _map PosScreenToWorld[mouseX,mouseY];
			["RequestSpecial", ["ParaAmmo",sideJoined,_callPos,clientTeam]] Call SendToServer;
		};
	};
	
	//--- Update the Artillery Status.
	if (paramArty) then {
		_fireTime = (Format["WFBE_FIREMISSIONTIMEOUT%1",(_currentUpgrades select 9)] Call GetNamespace);
	...
	if (paramArty && (_requestRangedList || time - _lastArtyUpdate > 3)) then {
		...
		_i = 0;
		{
			...
			_text = localize 'STR_WF_TACTICAL_ArtilleryInRange';
			if (_distance > _maxRange) then {_color = [0.875, 0, 0, 0.8];_text = localize 'STR_WF_TACTICAL_ArtilleryOutOfRange'};
			if (_distance <= _minRange) then {_color = [0.875, 0.5, 0, 0.8];_text = localize 'STR_WF_TACTICAL_ArtilleryRangeTooClose'};
