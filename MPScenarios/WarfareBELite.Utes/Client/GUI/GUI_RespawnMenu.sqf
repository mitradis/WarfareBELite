...
if (isNil 'WFBE_RespawnTime') then {
	WFBE_RespawnTime = missionNamespace getVariable "WFBE_RESPAWNDELAY";
	[] Spawn {
...
while {WFBE_RespawnTime > 0 && dialog && alive player} do {
	...
	if (time - _spawn_last_get > 1) then {
		...
		if (isNull _spawn_at_current) then {
			_spawn_at_current = [WFBE_DeathLocation, _spawn_locations] Call GetClosestEntity;
		};
		...
		//--- Add markers to the spawn if needed.
		{
			if !(_x in _spawn_locations_last) then {
				...
				[_spawn_markers, _marker] Call ArrayPush;
	...
	if (mouseButtonDown == 0 && mouseButtonUp == 0) then {
		...
		_nearest = [_clicked_at, _spawn_locations] Call GetClosestEntity;
...
if (alive player) then {
	...
	if (WFBE_RespawnTime > 0) then {
		...
		[] Spawn {
			sleep 1;
			if (WFBE_RespawnTime > 0) then {createDialog "WFBE_RespawnMenu"};
			//todo remove camera otherwise. & normal exit
		};
	} else {
		//--- Normal exit.
