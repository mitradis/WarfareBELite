...
if ((missionNamespace getVariable "WFBE_C_CAMPS_REPAIR_PRICE") > 0) then {
	...
	if ((Call GetPlayerFunds) < (missionNamespace getVariable "WFBE_C_CAMPS_REPAIR_PRICE")) exitWith {hint Format [localize "STR_WF_Repair_Camp_NoFunds", (missionNamespace getVariable "WFBE_C_CAMPS_REPAIR_PRICE") - (Call GetPlayerFunds)]};

	//--- Purchase a repair.
	-(missionNamespace getVariable "WFBE_C_CAMPS_REPAIR_PRICE") Call ChangePlayerFunds;
};
	
//--- Get the closest camp then.
_camp = [_vehicle, _camps] Call GetClosestEntity;
...
if (_camp_sideID != WFBE_Client_SideID) then {
	...
	[nil, "CampCaptured", [_camp, WFBE_Client_SideID, _camp_sideID, true]] Call SendToClients;
};
